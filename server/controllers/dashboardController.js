...
const dashboardController = {
  ...
  async getKeywords(req, res) {
    try {
      ...
      // 简单的关键词提取和统计
      const keywords = {}
      const stopWords = ['的', '了', '和', '与', '或', '在', '是', '都', '对', '这', '那', '有', '就', '我', '你', '他', '她', '它', '们']
      ...
      // 转换为词云数据格式
      const wordCloudData = Object.entries(keywords)
        .map(([name, value]) => ({ name, value }))
        .sort((a, b) => b.value - a.value)
        .slice(0, 50)  // 只取前50个关键词

      res.json(wordCloudData)
    } catch (error) {
      console.error('Error getting keywords:', error)
      res.status(500).json({ message: '获取关键词数据失败' })
    }
  },
