...
const reviewController = {
  ...
  // 获取评价历史记录
  getReviewHistory: async (req, res) => {
    try {
      const [history] = await db.query(`
        SELECT
          ...
          c.name as courseName,
          t.name as teacherName,
          e.teaching_attitude as teachingAttitude,
          e.teaching_content as teachingContent,
          e.teaching_method as teachingMethod,
          e.teaching_effect as teachingEffect,
          e.comment,
          DATE_FORMAT(e.create_time, '%Y-%m-%d %H:%i:%s') as submitTime
        FROM review_history rh
        JOIN administrators a ON rh.operator_id = a.id
        JOIN evaluations e ON rh.review_id = e.id
        JOIN courses c ON e.course_id = c.id
        JOIN teachers t ON c.teacher_id = t.id
  ...
  batchApprove: async (req, res) => {
    try {
      ...
      // 批量更新评价状态
      await db.query(
        'UPDATE evaluations SET status = ? WHERE id IN (?)',
        ['approved', ids]
      )

      // 批量记录操作历史
      const values = ids.map(id => [id, operatorId, 'approve', new Date()])
      await db.query(
        `INSERT INTO review_history
         (review_id, operator_id, action, create_time)
         VALUES ?`,
      ...
      await db.query('COMMIT')
      res.json({ message: '批量通过成功' })
    } catch (error) {
      await db.query('ROLLBACK')
      console.error('批量通过失败:', error)
      res.status(500).json({ message: '批量通过失败' })
    }
  },
  ...
  // 批量删除评价
  batchDeleteReview: async (req, res) => {
    try {
      const { ids } = req.body
      const operatorId = req.user.id
      ...
      // 批量记录操作历史
      const values = ids.map(id => [id, operatorId, 'delete', new Date()])
      await db.query(
        `INSERT INTO review_history
         (review_id, operator_id, action, create_time)
         VALUES ?`,
