...
export default {
  ...
  methods: {
    ...
    updateTrendChart (data) {
      console.log('开始更新趋势图表，数据:', data) // 添加日志
      ...
      console.log('处理后的数据:', { courseNames, averageScores }) // 添加日志
    ...
    async fetchCourseTrends () {
      try {
        const response = await axios.get('/admin/course-trends', {
          params: {
            page: this.trendCurrentPage,
            pageSize: 10
          }
        })
        this.trendTotal = response.data.total
        if (this.charts.trend) {
      ...
      try {
        ...
        if (this.charts.teacherScore) {
          const data = {
            ...
            pass: response.data.pass || 0,
            fail: response.data.fail || 0
          }
          this.updateTeacherScoreChart(data)
        }
      } catch (error) {
        console.error('获取课程评分数据失败:', error)
        this.$message.error('获取课程评分数据失败')
