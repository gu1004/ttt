...
-- 插入10个教师数据
INSERT INTO teachers (username, password, name, email, phone, department, title) VALUES
('teacher01', '123456', '张教授', 'zhang@example.com', '13800000001', '计算机科学系', '教授'),
('teacher02', '123456', '李教授', 'li@example.com', '13800000002', '软件工程系', '副教授'),
('teacher03', '123456', '王教授', 'wang@example.com', '13800000003', '信息工程系', '教授'),
('teacher04', '123456', '刘教授', 'liu@example.com', '13800000004', '计算机科学系', '副教授'),
('teacher05', '123456', '陈教授', 'chen@example.com', '13800000005', '软件工程系', '教授'),
('teacher06', '123456', '杨教授', 'yang@example.com', '13800000006', '信息工程系', '副教授'),
('teacher07', '123456', '赵教授', 'zhao@example.com', '13800000007', '计算机科学系', '教授'),
('teacher08', '123456', '黄教授', 'huang@example.com', '13800000008', '软件工程系', '副教授'),
('teacher09', '123456', '周教授', 'zhou@example.com', '13800000009', '信息工程系', '教授'),
('teacher10', '123456', '吴教授', 'wu@example.com', '13800000010', '计算机科学系', '副教授');
...
SELECT
    ...
    sc.course_id,
    -- 内容维度（20分）
    ROUND(5 + RAND() * 5, 1) as content_richness, -- 10分
    ROUND(2.5 + RAND() * 2.5, 1) as content_update, -- 5分
    ROUND(2.5 + RAND() * 2.5, 1) as content_organization, -- 5分

    -- 教学方法维度（20分）
    ROUND(5 + RAND() * 5, 1) as teaching_method_diversity, -- 10分
    ROUND(2.5 + RAND() * 2.5, 1) as teaching_interaction, -- 5分
    ROUND(2.5 + RAND() * 2.5, 1) as teaching_resource, -- 5分

    -- 教师维度（20分）
    ROUND(5 + RAND() * 5, 1) as teacher_attitude, -- 10分
    ROUND(2.5 + RAND() * 2.5, 1) as teacher_ability, -- 5分
    ROUND(2.5 + RAND() * 2.5, 1) as teacher_personality, -- 5分

    -- 课程维度（20分）
    ROUND(5 + RAND() * 5, 1) as course_objective, -- 10分
    ROUND(2.5 + RAND() * 2.5, 1) as course_difficulty, -- 5分
    ROUND(2.5 + RAND() * 2.5, 1) as course_pace, -- 5分

    -- 学习效果维度（20分）
    ROUND(5 + RAND() * 5, 1) as knowledge_grasp, -- 10分
    ROUND(2.5 + RAND() * 2.5, 1) as ability_improvement, -- 5分
    ROUND(2.5 + RAND() * 2.5, 1) as interest_stimulation, -- 5分
...
-- 生成评价历史记录
INSERT INTO review_history (evaluation_id, operator_id, action, reason, operate_time)
SELECT
    e.id as evaluation_id,
    (SELECT id FROM administrators WHERE username = 'admin') as operator_id,
    'approve' as action,
    CASE
        WHEN RAND() < 0.4 THEN '评价内容客观公正，符合要求'
        WHEN RAND() < 0.6 THEN '评价详细具体，对教学改进有帮助'
        WHEN RAND() < 0.8 THEN '反馈意见建设性强，通过审核'
        ELSE '评价符合规范，予以通过'
    END as reason,
    DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 7) DAY) as operate_time
FROM evaluations e
WHERE e.status = 'approved';

SELECT 'Review history inserted successfully' as message;
SELECT COUNT(*) as review_history_count FROM review_history;
...
SELECT
    ...
    (SELECT COUNT(*) FROM review_history) as review_history_count;
