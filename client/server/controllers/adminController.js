...
const adminController = {
  ...
  // 发送评价结果通知（发给教师）
  sendEvaluationResultNotification: async (courseId) => {
    try {
      ...
      await db.query(
        `INSERT INTO notifications (
          title, content, type, target_type, course_id, created_by
        ) VALUES (
          ?, ?, 'evaluation_result', 'teachers', ?, ?
        )`,
        [
          `课程《${course.name}》评价结果已出`,
          `您的课程《${course.name}》的学生评价结果已生成，请查看详情。`,
          courseId,
          req.user.id
        ]
      );
    } catch (error) {
      console.error('发送评价结果通知失败:', error);
    }
  },
};
